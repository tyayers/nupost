...
func InitializeProvider() data.PostIndex {
	...
	postBytes, err = dataProvider.DownloadFile("index_users_handles.json")

	if err == nil {
		json.Unmarshal(postBytes, &index.IndexUsersHandles)
	}
	...
	postBytes, err = dataProvider.DownloadFile("index_users_posts.json")

	if err == nil {
		json.Unmarshal(postBytes, &index.IndexUsersPosts)
	}
...
func FinalizeProvider(persistMode data.PersistMode, index data.PostIndex) {
	...
	if persistMode == data.PersistAll || persistMode == data.PersistOnlyUsers {
		...
		// Persist user handles
		jsonData, err = json.Marshal(index.IndexUsersHandles)
		if err != nil {
			fmt.Printf("could not marshal json: %s\n", err)
			return
		}

		dataProvider.UploadFile("index_users_handles.json", jsonData)
	}
	...
	// Persist users posts
	if persistMode == data.PersistAll {
		jsonData, err := json.Marshal(index.IndexUsersPosts)
		if err != nil {
			fmt.Printf("could not marshal json: %s\n", err)
			return
		}

		dataProvider.UploadFile("index_users_posts.json", jsonData)
	}
}
